	...
	[R['Mutagen Sample']]: [
		{
			...
			nodeId: undefined,
			boostersApply: false,
			note: 'The wiki lists Naeglar’s Hive Sabotage resource cache (Rotation B, ~15.5% for 3) as the only documented Mutagen Sample source on Eris. Resource caches roll from a reward table rather than enemy drops, so a Resource Drop Booster and the Steel Path passive don’t affect this pickup — the cache itself is the whole farm.',
		...
		},
		{
			...
			nodeId: undefined,
			boostersApply: false,
			note: 'Eris only has the one documented Mutagen Sample cache, so the late-game upgrade is running Naeglar in a full 4-player squad: each player’s own rotation-B completion rolls the cache table independently, multiplying caches found per run. It’s still a reward-table cache, not an enemy drop, so boosters and the Steel Path passive don’t change the per-cache odds.',
			source: 'https://wiki.warframe.com/w/Mutagen_Sample',
			lastVerified: '2026-07-06',
		},
	],
