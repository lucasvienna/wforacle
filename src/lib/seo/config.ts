...
// it. The `|| fallback` also guards an explicitly-empty value. A trailing
// slash is stripped so `SITE_URL + '/path'` never produces a double slash
// (e.g. a preview value like `https://example.com/`).
export const SITE_URL = (PUBLIC_SITE_URL || 'https://wforacle.avyiel.dev').replace(/\/+$/, '');
