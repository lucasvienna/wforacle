...
- **Sharing rules:** own ability kill = 100% to the Warframe; own weapon kill = 50/50 frame/weapon; a squadmate's kill = 25% to your frame + 75% split evenly across your _equipped_ weapons — so carrying a single leveling weapon triples its share vs a full loadout. Share range 50 m (250 m open worlds, 200 m Archwing, infinite in Railjack).
...
- **Steel Path has NO affinity bonus** — its +100% bonuses are resource & mod _drop chance_. SP farms win via +100 enemy levels feeding the level-scaled affinity multiplier (1 + 0.1425 × level^0.5) plus spawn density. The myth must be busted, not repeated.
